"""
core/providers/mistral_provider.py — Mistral AI (OpenAI-compatible).
Free trial credits. Get key: https://console.mistral.ai/api-keys
"""
...
class MistralProvider(BaseProvider):
    ...
    def _wrap(exc: Exception) -> RuntimeError:
        ...
        if "401" in msg or "unauthorized" in msg.lower():
            return RuntimeError("Invalid Mistral API key. Check Settings.")
        if "429" in msg:
            return RuntimeError("Mistral rate limit exceeded. Try again later.")
        if "model" in msg.lower() and "not found" in msg.lower():
            return RuntimeError("Model not available. Check model name in Settings.")
        return RuntimeError(f"Mistral error: {exc}")
