"""
core/providers/together_provider.py — Together AI (OpenAI-compatible).
$1 free credit on signup. Get key: https://api.together.ai/settings/api-keys
"""
...
class TogetherProvider(BaseProvider):
    ...
    def _wrap(exc: Exception) -> RuntimeError:
        ...
        if "401" in msg or "authentication" in msg.lower():
            return RuntimeError("Invalid Together AI key. Check Settings.")
        if "429" in msg:
            return RuntimeError("Together AI rate limit exceeded. Try again later.")
        if "model" in msg.lower() and "not found" in msg.lower():
            return RuntimeError("Model not available. Check model name in Settings.")
        return RuntimeError(f"Together AI error: {exc}")
